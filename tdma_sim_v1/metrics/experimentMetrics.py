...
from tdma_sim_v1.evaluators.distributedConsensusEvaluator import DistributedConsensusEvaluator
from tdma_sim_v1.metrics.experimentDto import AtomicData, AnomalyData, TopologyTestData, TopologyEvolutionTestData
...
class ExperimentMetrics:
    FILE_ANOMALIES = "result_anomalies.tsv"
    FILE_EXACT = "result_full.csv"
    FILE_TOPOLOGY = "result_per_topology.tsv"
    FILE_EVOLUTION = "result_per_topology_evolution.tsv"

    logger = logging.getLogger(__name__)
    evaluator = DistributedConsensusEvaluator()

    def __init__(self, reports_location: str):
        ...
        self.simulations_report = BufferedReportWriter(reports_location, self.FILE_EXACT, AtomicData.get_header())
        self.anomalies_report = BufferedReportWriter(reports_location, self.FILE_ANOMALIES, AnomalyData.get_header(), buffer_size=1)
        self.topology_report = BufferedReportWriter(reports_location, self.FILE_TOPOLOGY, TopologyTestData.get_header(), buffer_size=1)
        self.topology_evolution_report = BufferedReportWriter(reports_location, self.FILE_EVOLUTION, TopologyEvolutionTestData.get_header(), buffer_size=1)

    def record_simulation(self, experiment_id: Tuple[int, int, int, int, int, str], network: Network) -> None:
        data = _prepare_data_record(experiment_id, network)
    ...
    def record_simulation_details(self, experiment_id: Tuple[int, int, int, int, int, str], network: Network,
        ...
        data = _prepare_anomaly_record(experiment_id, network, valuation, reason, simulation_data)
...
def _prepare_anomaly_record(experiment_id: Tuple[int, int, int, int, int, str], network: Network, valuation: List[float], reason: str,
    ...
    if test_data:
        ...
        data.set_basic_info(network)
    data.set_construction_info(network, valuation)
    data.reason = reason
    return data


def _prepare_data_record(experiment_id: Tuple[int, int, int, int, int, str], network: Network) -> AtomicData:
    data = AtomicData()
    data.set_id(experiment_id)
    data.set_basic_info(network)
    return data
