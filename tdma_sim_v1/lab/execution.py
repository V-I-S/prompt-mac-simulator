...
from tdma_sim_v2.lab.extras.partitions import Partitions
...
class Execution:
    ...
    def perform(self) -> None:
        ...
        # extras
        # Partitions(self.config).plot()
        from tdma_sim_v2.lab.extras.comparison import Comparison
        Comparison(self, self.config).plot()
    ...
    def get_experiment_results(self, file: str, model: str, complete_file: str = None) -> Dict[str, List[float]]:
        ...
        if model == 'BtNode':
            df['step'] = df['step'].apply(lambda v: v / 1000.0)
    ...
    def _calculate_bt(self, failures_tolerance: int) -> Tuple[List[float], List[float]]:
        print(f'Adding calculated results for k={self.n_min_transmitters}, failures tolerance={failures_tolerance}')
        sampling = np.arange(0., 1., 0.002)
        probability_success = [
            fault_tolerant_bernoulli_success_probability(self.n_min_transmitters, self.n_slots, p, failures_tolerance)
            for p in sampling]
        return list(sampling), probability_success

    def _calculate_tslots(self, failures_tolerance: int) -> Tuple[List[float], List[float]]:
        print(f'Adding calculated results for k={self.n_min_transmitters}, failures tolerance={failures_tolerance}')
        sampling = np.arange(0, self.strategy_evolution_rng.stop, 5)
        probability_success = [
            fault_tolerant_tslots_success_probability(self.n_min_transmitters, self.n_slots, p, failures_tolerance)
            for p in sampling]
        return list(sampling), probability_success
